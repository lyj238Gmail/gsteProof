...
theory n_mutualEx_base imports newparaGste1
begin
...
let s = (parallel [( ((Para (Ident ''n'') i), (Const T)))]) in
guard g s"
...
let s = (parallel [( ((Para (Ident ''n'') i), (Const C))), ( ((Ident ''x''), (Const false)))]) in
guard g s"

definition n_Exit::"nat \<Rightarrow> rule" where [simp]:
"n_Exit  i\<equiv>
let g = (eqn (IVar (Para (Ident ''n'') i)) (Const C)) in
(*let s = (parallel [( ((Para (Ident ''n'') i), (Const E)))]) in*  -need rewrite 1*)
let s = (parallel [( ((Para (Ident ''n'') i), (Const E)))]) in
guard g s"

definition n_Idle::"nat \<Rightarrow> rule" where [simp]:
"n_Idle  i\<equiv>
let g = (eqn (IVar (Para (Ident ''n'') i)) (Const E)) in
let s = (parallel [( ((Para (Ident ''n'') i), (Const I))), ( ((Ident ''x''), (Const true)))]) in
guard g s"
