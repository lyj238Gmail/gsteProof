theory newparaGste1 imports Main 
begin
...
lemma forall1:
  shows "\<forall> i. i \<le> N \<longrightarrow>formEval Interp (forallForm (down N) pf ) s\<longrightarrow>formEval Interp (pf i) s" (is "?P N")  
proof(induct_tac N)
    show "?P  0"
      by simp
  next
    ...
    show "?P  (Suc N)"
      ...
      next
       ...
       proof(rule allI,(rule impI)+,cut_tac c2,simp)
         fix i
         assume d1:"i\<le> Suc N" and d2:" formEval Interp (pf (Suc N)) s \<and> formEval Interp (forallForm (N # down N') pf) s"
         have "i=Suc N \<or> i<Suc N" 
           by (cut_tac d1, arith)
         moreover
         {assume e1:"i=Suc N"
           have " formEval Interp (pf i) s"
         ...
         ultimately show "formEval Interp  (pf i) s"
           by blast
       qed
     qed
   qed
...
lemma forallLemma [simp,dest]:
  assumes a1:"i \<le> N" and a2:"formEval Interp (forallForm (down N) pf ) s"
  shows "formEval Interp  (pf i) s"
...
lemma andListFormAux1:
  ...
  shows "set fs \<subseteq>  fs' \<longrightarrow>( \<forall> inv. inv \<in> fs' \<longrightarrow>formEval I inv s)\<longrightarrow> formEval I  (andList fs) s"
  proof(induct_tac fs,auto)qed
  
lemma andListForm1[simp,intro]:
 
  "\<lbrakk>set fs \<subseteq>  invs ; ( \<forall> inv. inv \<in>invs \<longrightarrow>formEval I inv s)\<rbrakk>\<Longrightarrow> formEval I (andList fs) s"
 by(metis  andListFormAux1)  


lemma andList2Aux:
   shows "formEval I (neg frm) s \<longrightarrow> frm \<in>set frms \<longrightarrow> formEval I (neg (andList frms)) s"
   proof(induct_tac frms,auto) qed
...
lemma andList1Aux:
   shows "formEval I ( (andList frms)) s  \<longrightarrow> frm \<in>set frms \<longrightarrow> formEval I ( frm) s "
  using andList2 formEval.simps(4) by blast
   
lemma andList1:
   shows "formEval I  ( (andList frms)) s  \<Longrightarrow>  frm \<in>set frms \<Longrightarrow> formEval I ( frm) s " 
    by (metis andList1Aux)
lemma resAux1:
  assumes   b:"formEval I frm s" and c:"formEval I (neg (andList (frm#frms))) s"
  shows"formEval I (neg (andList frms)) s"
  apply(cut_tac b c)
  apply auto
  done
...
definition  varOfSent::" statement\<Rightarrow> varType set" where [simp]:
" varOfSent  ( Statement)=set (map fst (statement2Assigns Statement))"



definition  cat::"statement \<Rightarrow> statement \<Rightarrow>statement" where [simp]:
"cat S1 S2 =parallel ((statement2Assigns S1)@(statement2Assigns S2))"  

lemma varsOnCat[simp]:
  shows "varOfSent (cat (parallel S1) (parallel  S2))=(varOfSent (parallel S1) ) \<union> (varOfSent (parallel S2) )"
  apply(induct_tac S1,simp)
  by simp
...
lemma   forallVars:
  ...
  next
    ...
    have "fst ` set (statement2Assigns (forallSent (Suc n # down n) paraForm)) = 
    fst ` set (statement2Assigns (paraForm (Suc n))) \<union> fst ` set (statement2Assigns (forallSent (down n) paraForm))"
      apply simp
